...
const currentPageIndex = routes.findIndex(route => route.path === url.pathname)

type Navigation = Record<'prev' | 'next', typeof routes[number] | undefined>
let navigation: Navigation = {
  prev: routes[currentPageIndex - 1] || undefined,
  next: routes[currentPageIndex + 1] || undefined,
}
...
navigationElement.innerHTML =
  (!!navigation.prev
    ...
    : '') +
  (`<span class="fixed bottom-0 left-1/2 -translate-x-1/2 text-white bg-purple-500 px-4 py-1 rounded-t-lg">${routes[currentPageIndex].title}</span>`) +
  (!!navigation.next
